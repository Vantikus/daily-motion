...
- `progress.html` + `progress.js` — история, цели и резервные копии
...
## P3 — персонализация и история

- добавлен отдельный экран «Прогресс»;
- календарь активности за 28 дней и список последних активных дней;
- текущая и лучшая серия, завершённые комплексы и суммарное время;
- недельная цель от 1 до 7 тренировочных дней;
- Home показывает текущий прогресс недельной цели;
- тренировочный день засчитывается после завершения хотя бы одного комплекса;
- экспорт локального состояния в JSON;
- импорт резервной копии с полной заменой локального состояния;
- экран прогресса включён в offline app-shell и PWA shortcuts.
